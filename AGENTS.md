...
갱신: 2026-09-13. 사용자 1명이 `main`에서 개발·운영한다.
...
- 운영 주소: https://pawproof.kr/ 및 https://pawproof-rose.vercel.app/ — Vercel 배포 완료는 사용자 확인 기준이다.
- PWA·실 API 어댑터·Firebase 인증·프로필/반려견·여행 노트 자동 저장을 구현했다. 비회원 코스는 메모리 전용이며 새로고침 경고를 제공하고, 노트 저장은 로그인한 계정에 한정한다. 실제 모드는 카카오 지도 탐색에서 시작하며 여행 노트와 편집 상태를 공유한다. 집중 탐색 지역은 경기 북서부(고양·파주·양주)이며 기존 인천 보강 자산을 유지한다. 선정 근거·공식 데이터 결합 한계는 [지역 선정](docs/data/REGION_SELECTION_2026-09-16.md)에 있다.
...
- code-server는 `/absproxy/<port>`, 운영은 빈 basePath를 사용한다. `.env.example`은 운영 템플릿이며 실제 값은 비추적 환경 파일 또는 Vercel에만 넣는다.
- OAuth 팝업 전에 앱 모달의 포커스·스크롤 잠금을 해제한다. 리디렉션 인증 도입 시 Firebase의 동일 출처/저장소 제약을 먼저 검증한다.
- Firebase Web 설정은 공개 식별자로 구분하고, Admin·관광·LLM 자격증명은 서버 전용으로 관리하고 코드·문서·로그·커밋에 넣지 않는다. KTO 원문·파생 규정 저장은 데이터 전략의 허용 범위를 따른다.
...
- UI·인증 변경은 관련 Playwright 흐름을 검증한다. `pnpm test:firebase`는 로컬 demo 에뮬레이터를 사용한다. 실제 사용자 계정·유료 API를 회귀 검사에 쓰지 않는다.
- 사용자가 검증 후 **`main` 푸시와 그에 따른 Vercel 자동 배포를 승인했다.** 관련 변경을 작게 커밋하고 `git push origin main`까지 진행한다. 강제 푸시·무관한 변경·별도 인프라 변경은 포함하지 않는다.
- Vercel은 `pnpm build`, Node 24.x, package.json의 고정 pnpm을 사용한다. 빌드 스크립트 정책은 pnpm-workspace.yaml에 패키지별로 명시하며 전체 허용으로 우회하지 않는다.
- 푸시 성공과 배포 성공은 구분한다. 배포 후 운영 응답·변경 반영을 확인하고 검증 결과·커밋·남은 실기기 확인 사항을 보고한다. 자세한 절차는 [Vercel 안내](docs/operations/VERCEL_DEPLOYMENT.md)를 따른다.
