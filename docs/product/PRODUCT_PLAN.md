...
| 여행 범위 | 당일 여행, 방문지 1~5개 |
| 장소 | 관광지·식당·카페 중심 |
| 이동 | 자가용, 사용자가 선택한 장소 기준 |
| 집중 지역 | 서울 명동을 기본 탐색·시연 지역으로 사용하고, 경기 북서부·인천 검색·보강 자산은 보조 권역으로 유지한다. 서울 KTO 표본·공식 출입 기준·참고 출처는 [서울 명동 데이터 보강](../data/SEOUL_MYEONGDONG_COVERAGE_2026-09-18.md)에 기록한다. |
...
```mermaid
flowchart TD
    A[반려견 정보와 여행 날짜 입력] --> B[방문지 1~5개 선택]
    B --> C[동반 조건과 방문 시간 검증]
    C --> D[문제 방문지와 근거 표시]
    D --> E[준비물·구역·시간 변경 검토]
    D --> F[같은 목적의 대체 장소 비교]
...
## 모바일 지도에서 코스 담기 (2026-09-17)

실제 모드의 모바일 화면은 큰 지도를 기본으로 하고 지도/여행 노트 전환을 하단 내비게이션에 둔다. 마커 또는 목록의 장소를 고르면 하단 패널에 주소·동반 조건·코스 담기를 보여준다. 장소를 담아도 지도에 머무르며 패널을 닫아 다음 후보를 고를 수 있다. 완료 버튼은 동일한 편집 상태의 여행 노트를 연다. 기존 계정 자동 저장과 비회원 메모리 전용 계약을 유지하므로 완료는 별도의 영구 저장 확약이 아니다.

기존 마커 숫자는 목록 순번이었다. 이제 개별 후보는 이름, 겹친 후보 묶음은 `N곳`으로 표시한다. 묶음 선택 시 모든 구성원을 목록에서 확인하고 담을 수 있으며 확대하면 화면 좌표 기준으로 다시 나뉜다. 같은 좌표는 최대 확대에서도 전체 구성원 목록을 제공한다. 후보는 KTO 검색 결과이며 가맹점 수·지역 전체 업소 수를 의미하지 않는다.

담은 장소는 검색/필터 변경 후에도 지도에 남기며 `코스 N`과 방문 순서 점선·전체 보기로 표시한다. 점선은 실제 도로 길찾기가 아니다. 예상 자가용 이동시간은 기존 여행 노트의 코스 검사에서 계산한다.
