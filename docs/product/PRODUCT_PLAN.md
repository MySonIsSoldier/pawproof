...
구현 우선안은 Next.js 기반 모바일 웹이며, 무료 호스팅·DB와 현재 code-server 개발 환경을 반영한다. 구체적 선택은 [기술 스택](../engineering/STACK_DECISION.md)에서 관리한다. 디자인 레퍼런스는 사용자가 제공하며 새 도메인 구매 여부도 사용자가 결정한다. 현재 화면 목록은 정보와 행동의 기획이며 최종 시각 디자인은 아니다.
