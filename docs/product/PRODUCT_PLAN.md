...
| 집중 지역 | 인천 우선 검토. 데이터·대체 후보 밀도 확인 후 권역 확정 |
| 플랫폼 | 모바일 우선 웹, 핵심 비로그인 체험 |
...
- 주요 행동: `내 코스 검사하기`.
- 보조 행동: `예시 코스로 체험하기`.
- 예시 코스도 실제 API·동일한 검증 로직을 사용한다. 가상 사례는 별도 표시한다.
...
채택한 구현 방향은 Next.js 기반 모바일 웹이며, 무료 호스팅·DB와 현재 code-server 개발 환경을 반영한다. 구체적 선택은 [기술 스택](../engineering/STACK_DECISION.md)에서 관리한다. 디자인 레퍼런스는 사용자가 제공하며 새 도메인 구매 여부도 사용자가 결정한다. 현재 화면 목록은 정보와 행동의 기획이며 최종 시각 디자인은 아니다.
