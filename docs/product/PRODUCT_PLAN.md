...
| 집중 지역 | 인천. 지원 업종 검색 43곳·시 자료 연결 9곳. 규정 완전성·유효 대체 밀도는 추가 확보 필요 |
...
## 선택형 계정과 여행 노트 (2026-09-12 추가)

핵심 코스 검사는 계속 비로그인으로 제공한다. 사용자가 Google 또는 이메일로 로그인하고 명시적으로 저장한 노트는 다른 기기에서도 불러올 수 있다. 계정 노트는 제목·반려견 프로필·일정·방문지 ID와 입력 조건에 한정하며 판정 결과는 다시 계산한다. 최대 20개 저장, 수정·삭제, 다른 기기의 수정 충돌 확인을 제공한다. 계정 변경 시 이전 계정 목록을 숨기고 자동으로 입력을 업로드하거나 교체하지 않는다. 설정·데이터 경계는 [Firebase 기준](../engineering/FIREBASE_AUTH_AND_STORAGE.md)에 기록한다.
