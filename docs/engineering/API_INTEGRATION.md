...
카카오는 Kakao Developers REST API 키와 자동차 길찾기 사용 권한이 필요하다. 보행시간 집계에는 같은 REST 키의 보행 경로 권한도 필요하며, 권한이 없으면 도보시간을 추정하지 않고 미확정으로 남긴다. 실제 장소 여행 노트의 지도는 방문 순서와 경로를 보기 전용으로 표시하고, 이동시간은 코스 검사 완료 후 서버에서 조회한다. 자동차 시간은 현재 교통 기준이며 미래 여행일의 교통 예측값이 아니다.
...
- Kakao 자동차: https://apis-navi.kakaomobility.com/v1/directions. Kakao 보행: https://dapi.kakao.com/v2/routing/walk. 두 응답의 초를 분으로 올림하며, 실패 또는 좌표 누락은 미확인으로 남긴다. 자동차 시간은 일정 도착시각 계산에 사용하고, 도보 시간은 코스 이동 참고 집계로만 표시한다.

카카오 공개 무료 쿼터 기준(변경 가능): 자동차 길찾기 REST API는 일 10,000건, 카카오맵 REST 도보 경로는 일 1,000건이다. 최대 5곳 코스는 인접 구간 4개를 자동차·도보로 각각 조회하므로 검사 1회당 최대 4건씩 사용한다. 구간 토글은 검사 결과를 재사용해 추가 호출하지 않는다.
...
- [카카오 보행 경로](https://developers.kakao.com/docs/en/kakaomap/rest-api)
