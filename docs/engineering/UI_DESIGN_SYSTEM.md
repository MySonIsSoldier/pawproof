...
지도 중심·검색 반경·필터·선택·현재 규정 색인은 화면별 Zustand, 주변 검색/여행지 검색/규정 요청은 TanStack Query가 담당한다. 지도 SDK 인스턴스·DOM 오버레이·이벤트는 KakaoMapView 내부 ref/effect에서 생성·정리한다. 지도↔노트는 동일 PlannerProvider를 유지하며 숨겨진 화면에서 주변 API를 새로 조회하지 않는다. SDK 실패 시 목록과 명시적인 재시도를 제공한다. 모바일 패널은 ‘목록 크게/지도 크게’ 버튼으로 조절하고 공통 Dialog로 프로필을 편집한다.
