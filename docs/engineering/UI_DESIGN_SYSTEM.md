...
- 비회원의 여행 입력은 Zustand 메모리에만 둔다. useGuestLeaveWarning은 편집/검사 후 브라우저 beforeunload 확인창을 요청한다. useCloudTrips는 인증된 계정의 자동 저장·복원·미전송 초안을 담당한다. 기기 저장 코드와 UI는 제거했고 기존 pawproof.trip.v1 데이터를 읽거나 이전하지 않는다.
...
## 상단 여행 노트와 계정 전용 저장 (2026-09-13)

CloudTrips는 모드 안내 위에 위치한다. 비회원은 손실 안내와 로그인 CTA, 회원은 노트 제목 입력·자동 저장 상태·목록·새 노트를 제공한다. NoteLibrary는 shadcn/Radix Dialog의 포커스·스크롤 경계를 사용하고 제목/날짜 검색·현재 노트 표식·삭제 확인을 제공한다. 모바일에서는 제목 아래 버튼을 나란히 배치한다. 정상 열기는 자동 저장 후 진행하며 저장 실패가 있으면 명시적인 초안 폐기 확인을 거친다. 저장 요청 오류는 상단에 유지하고 실패 시에만 재시도 버튼을 노출한다.

PlannerProvider는 비회원 → 로그인에서 현재 스토어를 채택하고, 기존 계정 → 로그아웃/다른 계정에서는 새 스토어를 생성한다. 계정 변경 때 URL의 note도 제거한다. 회원 자동 저장 성공 후에는 새로고침 경고가 없으며 미전송 변경에는 기존 경고를 유지한다. 브라우저 기본 경고와 앱 내 목록/삭제 모달의 역할을 구분한다.

## 지도 화면 (2026-09-16)

지도 중심·검색 반경·필터·선택·현재 규정 색인은 화면별 Zustand, 주변 검색/여행지 검색/규정 요청은 TanStack Query가 담당한다. 지도 SDK 인스턴스·DOM 오버레이·이벤트는 KakaoMapView 내부 ref/effect에서 생성·정리한다. 지도↔노트는 동일 PlannerProvider를 유지하며 숨겨진 화면에서 주변 API를 새로 조회하지 않는다. SDK 실패 시 목록과 명시적인 재시도를 제공한다. 모바일은 큰 지도와 비모달 하단 장소 패널을 사용하고 공통 Dialog로 프로필을 편집한다. 2026-09-17 후속 기준을 아래에 기록했다.

## 모바일 지도 패널과 내비게이션 (2026-09-17)

반경은 공통 Radix Select를 사용한다. 모바일 지도는 고정 비율의 상하 분할 대신 지도 전체 높이를 사용하고, `ResponsiveSheet`는 데스크톱 aside / 모바일 Radix 비모달 Dialog를 제공한다. 배경 지도의 조작을 유지하며 닫기·Escape·목록 버튼 포커스 복귀를 지원한다. `useMobile`은 useSyncExternalStore로 구독하고 SSR에서는 데스크톱 스냅샷을 사용한다. 패널 표면·모션은 공통 CSS, 후보/코스와 하단 내비게이션 배치는 explore.css가 소유한다.

여행 작업 내비게이션은 720px 이하에서 safe-area를 포함한 하단 고정 두 항목으로 표시한다. 노트 검사 바는 내비게이션 위에 배치한다. 지도 검색 버튼은 가로 중심 위치를 CSS translate로 분리하여 공통 hover transform과 충돌하지 않게 한다. 지도 아래 코스 요약·출처·여백을 제공한다.

장소 상세는 장소를 선택할 때 자동으로 규정을 조회하며, 조회 중에는 로딩 문구를 표시한다. 모바일 지도 도구는 검색 버튼 하나로 검색·필터 drawer를 연다. KTO 필드명은 사용자용 근거 문구에서 숨긴다.
