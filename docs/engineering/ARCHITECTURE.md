...
주변 후보 검색과 규정 상세 확인을 분리한다. discovery API는 좌표·반경 검색과 명시적인 최대 5곳 조회를 제공하며, 지도 이동만으로 LLM을 호출하지 않는다. 규정 평가·문의 문구 생성은 순수 도메인 함수다. 카카오 지도 SDK는 지도 화면에서만 로드한다. 활성 화면의 규정 메모리와 계정 노트 저장 범위를 구분한다. 구현 단계는 [지도 작업 계획](../delivery/MAP_DISCOVERY_PLAN.md)에 기록한다.
