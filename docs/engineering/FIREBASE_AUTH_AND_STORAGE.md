...
기준일: 2026-09-12. 코드 구현과 에뮬레이터 검증, 실제 Firebase 프로젝트 연결을 구분한다. 실제 설정값은 저장소에 기록하지 않는다.
...
5. Authentication의 비밀번호 정책은 최소 12자 이상으로 설정하는 것을 권장한다. 이메일 열거 방지 설정을 유지한다. 인증·재설정 메일 템플릿의 서비스 이름과 언어도 확인한다.
