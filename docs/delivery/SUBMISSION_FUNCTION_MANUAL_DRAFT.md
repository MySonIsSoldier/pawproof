...
`#반려견맞춤형` `#LLM규정구조화` `#코스복구` `#관광데이터활용` `#PWA접근성`
...
한국관광공사 반려동물 동반여행 API의 장소·상세·동반 정보를 조회하고, 필요한 경우 식품안전나라 공식 음식점 목록을 함께 확인한다. OpenRouter LLM이 규정 원문에서 입장 가능, 체중, 마릿수, 구역, 장비, 예방접종, 운영시간 정보를 구조화하고 원문 근거를 보존한다. 이후 결정론적 규칙 엔진이 반려견 조건과 대조해 최종 상태를 판정한다. 정보가 없거나 충돌하면 이용 가능으로 추정하지 않고 `확인 필요`로 남긴다.
...
![#LLM규정구조화 기능 흐름도](../../public/submission/flows/flow-policy-verification.svg)

실제 제품 화면: 장소 선택 후 LLM이 정리한 공식 근거와 조회 시각, `반려견 출입 가능`, `확인 필요`가 함께 표시되는 화면

![#LLM규정구조화 실제 제품 화면](../../public/submission/product/hash-policy-verification.png)
...
아래 문구는 첨부 양식의 `해시태그1~5`, `연계기능1~5`, `기능설명1~5` 칸에 그대로 옮겨 적을 수 있는 최종안이다. 각 해시태그의 흐름도는 서로 다른 문제와 결과를 보여주도록 분리한다.

#### 1번

- 해시태그1: `#반려견맞춤형`
- 연계기능1: 반려견별 조건 입력·복수 반려견 코스 판정
- 기능설명1: 함께 가는 반려견의 체중·견종·마릿수와 준비물을 입력하면 장소별 동반 규정 및 실내·야외 조건과 대조해 반려견별 결과와 방문 전 준비사항을 보여주는 기능입니다.
- 기능 흐름도1: `[반려견 직접 입력 또는 프로필 선택] → [여러 마리·준비물·이용 구역 입력] → [장소 규정과 반려견 조건 대조] → [장소별 출입 가능·준비 필요·확인 필요 결과]`
- 첨부 화면1: 반려견 입력 화면과 코스 검사 결과의 방문지 카드

#### 2번

- 해시태그2: `#LLM규정구조화`
- 연계기능2: LLM 기반 공식 장소 규정 구조화·근거·문의 문구 생성
- 기능설명2: 사용자가 장소를 선택한 시점에 공식 안내 원문을 LLM이 출입 구역·체중·마릿수·준비물·예외 조건으로 구조화하고, 서버가 근거를 검증한 뒤 결정론적 규칙으로 상태를 판정해 출처·조회 시각·사람이 바로 사용할 문의 문구를 함께 보여주는 기능입니다.
- 기능 흐름도2: `[장소 선택] → [KTO·식품안전나라 공식 원문 조회] → [OpenRouter LLM이 조건·예외·근거 구조화] → [서버 형식·근거 검증 및 규칙 판정] → [출입 가능 뱃지·확인 필요·준비사항·문의 문구]`
- 첨부 화면2: 장소 상세의 출처·조회 시각·출입 가능/확인 필요 뱃지와 문의 문구

#### 3번

- 해시태그3: `#코스복구`
- 연계기능3: 이용 불가 방문지 대체·전체 코스 재검증
- 기능설명3: 코스에서 이용 불가로 판정된 장소만 대체 후보를 제안하고, 사용자가 후보를 선택하면 방문 순서·도착 시각·차량 및 도보 구간시간·전체 판정을 다시 계산해 여행 노트를 복구하는 기능입니다.
- 기능 흐름도3: `[코스 검사] → [이용 불가 방문지 확인] → [이용 가능·준비 필요·확인 필요 후보 비교] → [대체 장소 적용] → [전체 순서·차량/도보 시간·규정 재검증] → [여행 노트 반영]`
- 첨부 화면3: 코스 검사 결과, 대체 후보 비교, 구간별 이동시간이 표시된 여행 노트

#### 4번

- 해시태그4: `#관광데이터활용`
- 연계기능4: 한국관광공사 OpenAPI·식품안전나라 공식 장소 데이터 결합 지도
- 기능설명4: 한국관광공사 반려동물 동반여행 OpenAPI를 장소 후보·동반 정보의 기본 데이터로 사용하고, 식품안전나라 공식 목록을 음식점 후보로 보강해 지도·규정 조회·여행 노트까지 연결하는 기능입니다.
- 기능 흐름도4: `[서울 명동 기본 위치 또는 사용자 검색] → [KTO 반려동물 여행 API 조회] + [식품안전나라 공식 목록 병합] → [좌표·중복·출처 정규화] → [카카오 지도에 후보 표시] → [선택 장소 상세 조회 및 코스 담기]`
- 첨부 화면4: 서울 명동 지도에서 KTO·식품안전나라 장소 후보를 탐색하고 코스에 담는 화면

#### 5번

- 해시태그5: `#PWA접근성`
- 연계기능5: 모바일·데스크탑 설치형 웹앱과 여행 노트 재진입
- 기능설명5: 회원가입이나 앱스토어 설치 없이 브라우저에서 바로 시작하고, 지원 기기에서는 홈 화면에 설치해 지도 중심 탐색과 여행 노트를 앱처럼 다시 이용할 수 있도록 하는 기능입니다.
- 기능 흐름도5: `[브라우저에서 즉시 시작] → [모바일은 지도 중심·데스크탑은 지도와 노트 병행] → [홈 화면에 PWA 설치] → [같은 기기에서 여행 노트 재진입] → [오프라인에서는 규정을 추측하지 않고 연결 상태 안내]`
- 첨부 화면5: 모바일 지도 중심 화면과 홈 화면 설치/재진입 안내 화면
