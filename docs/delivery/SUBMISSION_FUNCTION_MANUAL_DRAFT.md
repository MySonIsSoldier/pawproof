...
3. **LLM은 문장을 정리하고 판정은 규칙으로 한다.** OpenRouter는 원문에서 확인 항목을 추출·문의 문구로 정리하는 보조 역할만 맡고, 최종 상태는 검증된 규정과 결정론적 규칙으로 계산한다.
...
| 의미 정리 | OpenRouter가 원문 필드와 누락 조건을 사용자 문장으로 변환 | 업체에 바로 물어볼 수 있는 문의 문구 |
