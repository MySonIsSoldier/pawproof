...
export async function verifyTrip(
  ...
  const documents = new Map<
    string,
    { document: PlaceDocument; policy: Policy }
  >();
  ...
  await Promise.all(
    [0, 1].map(async () => {
      for (let visit = pending.shift(); visit; visit = pending.shift()) {
        ...
        try {
          document = await providers.places.get(visit.placeId);
        } catch {
          document = {
            place: {
              id: visit.placeId,
              ...
              lat: 0,
              lng: 0,
              source: input.mode === "demo" ? "demo" : "kto",
            },
            raw: "",
            modifiedAt: null,
            fetchedAt: now.toISOString(),
            sourceUrl: null,
            sourceLabel: "원문 조회 실패",
          };
        }
        let policy: Policy;
        try {
  ...
  for (const visit of input.visits) {
    ...
    if (travelMinutes === null)
      findings.push({
        ...
        status: "confirm",
        message:
          "자동차 이동시간을 확인하지 못했어요. 이후 도착 시각은 미확정이에요.",
        quote: null,
        needs: [],
      });
    const departure: number | null =
      cursor === null ? null : cursor + visit.duration;
    visits.push({
      visit,
