...
The source may present these fields with user-facing Korean labels: 동반 가능 구역=acmpyTypeCd, 동반 가능한 반려동물=acmpyPsblCpam, 방문객 준비사항=acmpyNeedMtr, 안전 안내=relaAcdntRiskMtr, 추가 동반 안내=etcAcmpyInfo, 동반 구역=acmpyZone, 현장에서 제공하는 시설=relaPosesFclty, 현장에서 제공하는 물품=relaFrnshPrdlst. Keep the Korean label out of user-facing messages when it only introduces an action; state the action itself.
...
When an entry denial names only an indoor area (for example, 가옥 안쪽은 동반 불가 or 실내 출입 제한), set scope=indoor rather than all. Keep an outdoor visit available when the source also says some areas allow companion dogs.
