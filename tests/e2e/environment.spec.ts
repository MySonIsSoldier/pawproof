...
test("HTML, CSS, JS, client navigation, deep refresh and health route", async ({
  ...
  page.on("response", (response) => {
    if (response.url().includes("/_next/") && response.url().includes(".js")) {
      ...
      if (
        response.status() !== 304 &&
        (response.status() !== 200 ||
          !/javascript/u.test(response.headers()["content-type"] || ""))
      )
        errors.push("Invalid script response");
  ...
  await page.reload();
  await expect(
    page.getByRole("heading", { name: "PawProof 소개" }),
  ).toBeVisible();
  await page.getByRole("main").getByRole("link", { name: "처음으로" }).click();
  await expect(page.getByRole("heading", { level: 1 })).toContainText(
    "우리 강아지",
  );
  ...
  expect(health).toEqual({
    status: 200,
    body: { status: "ok", service: "pawproof" },
  });
...
test("development-only browser utility check and HMR preserve client state", async ({
  page,
}) => {
  ...
  const sourceFile = new URL(
    "../../src/features/development/environment-check.tsx",
    import.meta.url,
  );
  ...
  try {
    ...
    await receivedFrame;
    await expect(
      page.getByRole("heading", { name: "개발 환경 점검 · HMR", exact: true }),
    ).toBeVisible();
    await expect(page.getByRole("status")).toHaveText("API 정상 응답 1회");
  } finally {
    if ((await readFile(sourceFile, "utf8")) === changed)
      await writeFile(sourceFile, original);
  }
  await expect(
    page.getByRole("heading", { name: "개발 환경 점검", exact: true }),
  ).toBeVisible();
  expect(errors).toEqual([]);
});

test("production hides diagnostics and serves public assets at the root", async ({
  page,
  request,
}) => {
