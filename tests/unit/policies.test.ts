...
test("individual pet restriction, count and all findings are retained", () => {
  ...
  const findings = evaluatePolicy(policy, {
    ...context,
    pets: [...context.pets, { name: "콩", breed: "래브라도", weight: 20 }],
    equipment: [],
  });
...
test("missing count cannot be promoted to unrestricted", () =>
  assert.equal(
...
test("indoor restriction does not leak to outdoor", () => {
  const policy = demoPolicy("demo-table");
  policy.rules.find((r) => r.kind === "weight")!.scope = "indoor";
  const findings = evaluatePolicy(policy, {
    ...context,
    zone: "outdoor",
    pets: [{ ...context.pets[0], weight: 20 }],
  });
...
test("stay through closing, unknown travel and Korean weekday", () => {
  assert.equal(
    summarize(
      evaluatePolicy(demoPolicy("demo-forest"), { ...context, arrival: 1130 }),
    ),
    "blocked",
  );
  assert.equal(
    summarize(
      evaluatePolicy(demoPolicy("demo-forest"), { ...context, arrival: null }),
    ),
    "confirm",
  );
...
test("schema rejects invalid date, duplicate IDs and mixed data modes", () => {
  ...
  assert.equal(
    tripSchema.safeParse({
      ...trip,
      visits: trip.visits.map(() => trip.visits[0]),
    }).success,
    false,
  );
});
test("fabricated evidence and malformed semantic values rejected", () => {
  ...
  assert.throws(() =>
    validateExtraction(
      { ...value, rules: [{ ...policy.rules[1], operator: "any" }] },
      policy.raw,
    ),
  );
});
...
test("locked place cannot be replaced and subsequent appointment is preserved", async () => {
  ...
  trip.visits[1].locked = false;
  trip.visits[2].locked = true;
  const providers = demoProviders();
  providers.travel.minutes = async (a, b) =>
    a.id === "demo-garden" ||
    b.id === "demo-garden" ||
    a.id === "demo-picnic" ||
    b.id === "demo-picnic"
...
test("extraction or travel failure never becomes an available result", async () => {
  ...
  assert.equal(result.visits[1].arrival, null);
  assert.equal(result.totalTravel, null);
});

test("preparation groups repeated items and preserves OR alternatives", async () => {
  const { buildPreparation } =
    await import("../../src/domain/itinerary/preparation.ts");
  const trip = createDemoTrip();
  trip.equipment = [];
  const result = await verifyTrip(trip, demoProviders());
  const tasks = buildPreparation(result);
  assert.equal(tasks.find((task) => task.label === "목줄")?.sources.length, 4);
  assert.equal(tasks.filter((task) => task.label === "목줄").length, 1);
  assert.ok(tasks.some((task) => task.label.includes("또는")));
});
